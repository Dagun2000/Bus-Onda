...
router.post('/auth/device', (req, res) => {
  ...
  if (!deviceId)
    return res.status(400).json(wrap(false, null, { code: 'VALIDATION_ERROR', message: 'deviceId is required' }));
    ...
    if (bus.busNumber === lineNo && bus.ws?.readyState === 1) {
      bus.ws.send(
        JSON.stringify({
          type: 'command',
          cmd: 'ride_request',
          payload: { deviceId, lineNo, direction, userLocation, requestId: request.requestId },
        })
      );
    }
  }
...
router.post('/ride/request', (req, res) => {
  ...
  res.json(wrap(true, { status: 'PENDING' }));
});
